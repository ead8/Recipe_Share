...
import {UPDATE_RECIPE,GET_CATEGORIES,GET_RECIPES_BY_PK} from '../../../appolo/appolo.js'
...
watchEffect( async() => {
  const { data: recipeData } = await useAsyncQuery(GET_RECIPES_BY_PK, { id });

  const recipe = recipeData?._value?.recipes_by_pk;
  title.value = recipe.title;
  description.value = recipe.description;
  timeToPrepare.value = recipe.preparation_time.toString();
  foodCategory.value = recipe.category_id;
  steps.value = recipe.steps.map((step) => ({ description: step }));
  ingredients.value = recipe.ingredients.map((ingredient) => {
  if (ingredient) {
    const [name, quantity] = ingredient.split(' (');
    return { name, quantity: quantity.slice(0, -1) };
  } else {
    return { name: '', quantity: '' }; 
  }
});
selectedImages.value = recipe.images.map((url) => ({ url }));
const previouslyFeaturedImageUrl = recipe.featured_image;
featuredImageIndex.value = selectedImages.value.findIndex(image => image.url === previouslyFeaturedImageUrl);
...
const submitRecipe = async () => {
  try {
    ...
    const { mutate, error } = useMutation(UPDATE_RECIPE, { variables });
...
<template>
  <div class="container mx-auto p-4 bg-blue-100">
    <VForm @submit="submitRecipe" class="bg-slate-100 rounded-lg shadow-lg p-4" :validation-schema="recipeSchema" v-slot="{ meta }">
      <h1 class="text-3xl font-bold mb-4 text-blue-700 mt-14">Edit your Recipe</h1>
      ...
      <div class="mb-6">
        ...
        <VField v-model="title" name="title" type="text" class="w-full rounded-md p-2 mb-2" placeholder="Title" />
          ...
          <VField v-model="description" name="description" class="w-full h-20 rounded-md p-2 mb-2" placeholder="Description"></VField>
          <VErrorMessage name="description" class="text-red-500" />
          <VField v-model="timeToPrepare" name="timeToPrepare" type="text" class="w-1/4 rounded-md p-2 mb-2 mr-4" placeholder="Time to Prepare in minutes" />
     ...
     <div class="mb-6">
        ...
        <label class="bg-blue-500 text-slate-100 px-4 py-2 rounded-md cursor-pointer">
          Choose File
          <input type="file" multiple @change="handleImageChange" class="hidden" />
        </label>
        ...
        <div v-for="(image, index) in selectedImages" :key="index">
          ...
          <label class="block mt-2">
            <input
              type="radio"
              :value="index"
              v-model="featuredImageIndex"
              class="mr-2 cursor-pointer"
            />
            <span class="text-blue-700">Featured</span>
          </label>
          <button @click="removeSelectedImage(index)" class="mt-2 bg-red-500 text-slate-100 px-4 py-2 rounded-md slate-100">Remove Image</button>
        </div>
      </div>
    </div>
      ...
      <div class="mb-6">
        ...
        <div v-for="(step, index) in steps" :key="index" class="flex items-center mb-2">
          <VField v-model="step.description" type="textarea" class="w-full rounded-md p-2" :name="`steps[${index}].description`" placeholder="Step Description"></VField>
          <VErrorMessage :name="`steps[${index}].description`" class="text-red-500" />
          <button @click="removeStep(index)" class="ml-2 bg-red-500 text-slate-100 px-4 py-2 rounded-md slate-100 whitespace-nowrap">Remove Step</button>
        </div>
      ...
      <div class="mb-6">
        ...
        <div v-for="(ingredient, index) in ingredients" :key="index" class="flex items-center mb-2">
          <VField v-model="ingredient.name" type="text" class="w-1/2 rounded-md p-2 mr-2" :name="`ingredients[${index}].name`" placeholder="Ingredient Name" />
            <VErrorMessage :name="`ingredients[${index}].name`" class="text-red-500" />
            <VField v-model="ingredient.quantity" type="text" class="w-1/4 rounded-md p-2 mr-2" :name="`ingredients[${index}].quantity`" placeholder="Quantity" />
            <VErrorMessage :name="`ingredients[${index}].quantity`" class="text-red-500" />
          <button @click="removeIngredient(index)" class="bg-red-500 text-slate-100 px-4 py-2 rounded-md ">Remove Ingredient</button>
        </div>
        <button @click="addIngredient" class="bg-blue-500 text-slate-100 px-4 py-2 rounded-md">Add Ingredient</button>
      </div>
      <!-- Submit Button -->
      <button type="submit" :disabled="isSubmitting" class="bg-green-500 text-slate-100 px-6 py-2 rounded-md hover:bg-green-600 transition duration-300">
